    ...
    while (flag1 == 1) {
        try {
            ...
            for (int i = 0; i < cena.length(); i++) {
                if (i == 0 && cena[i] == '-') {
                    if(flag2==1)
                        throw invalid_argument("некорректное число (несколько минусов).");
                    flag2 = 1;
                }
                else if (cena[i] == '.') {
                    if (flag3 == 1)
                        throw invalid_argument("некорректное число (несколько точек).");
                    flag3 = 1;
                }
                else if(cena[i] != '0' && cena[i] != '1' && cena[i] != '2' && cena[i] != '3' && cena[i] != '4' && cena[i] != '5' && cena[i] != '6' && cena[i] != '7' && cena[i] != '8' && cena[i] != '9')
                    throw invalid_argument("некорректное число (символы, являющиеся вторым минусом или не являющиеся цифрами или точкой).");
            }
            if(cena == "-" || cena == "." || cena =="-." || cena == "")
                throw invalid_argument("некорректное число (отсутствуют цифры).");
            ...
            for (int i = 0; i < kolvo.length(); i++) {
                if (i == 0 && kolvo[i] == '-') {
                    ...
                    throw invalid_argument("некорректное число (символы, являющиеся вторым минусом или не являющиеся цифрами).");
            }
            if (kolvo == "-" || kolvo == "")
                throw invalid_argument("некорректное число (отсутствуют цифры).");
