    ...
    while (flag1 == 1) {
        try {
            ...
            for (int i = 0; i < cena.length(); i++) {
                if (i == 0 && cena[i] == '-') {
                    if(flag2==1)
                        throw domain_error("некорректное число (несколько минусов).");
                    flag2 = 1;
                }
                else if (cena[i] == '.') {
                    if (flag3 == 1)
                        throw domain_error("некорректное число (несколько точек).");
                    flag3 = 1;
                }
                else if(cena[i] != '0' && cena[i] != '1' && cena[i] != '2' && cena[i] != '3' && cena[i] != '4' && cena[i] != '5' && cena[i] != '6' && cena[i] != '7' && cena[i] != '8' && cena[i] != '9')
                    throw domain_error("некорректное число (символы, не являющиеся цифрами, точкой или минусом).");
            }
            if(cena == "-" || cena == "." || cena =="-." || cena == "")
                throw domain_error("некорректное число (отсутствуют цифры).");
            ...
            for (int i = 0; i < kolvo.length(); i++) {
                if (i == 0 && kolvo[i] == '-') {
                    ...
                    throw domain_error("некорректное число (символы, не являющиеся цифрами или минусом).");
            }
            if (kolvo == "-" || kolvo == "")
                throw domain_error("некорректное число (отсутствуют цифры).");
